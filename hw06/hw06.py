...
class VendingMachine:
    ...
    def restock(self, quantities):
        self.quantities += quantities
        return f"Current {self.name} stock: {self.quantities}"

    def vend(self):
        if self.balance >= self.price and self.quantities > 0:
            self.balance-=self.price
            if self.balance > 0:
                delta = self.balance
                self.balance = 0
                self.quantities -= 1
                return f'Here is your {self.name} and ${delta} change.'
            else:
                self.quantities -= 1
                return f'Here is your {self.name}.'
        else:
            if self.quantities > 0:
                delta = self.price - self.balance
...
def store_digits(n):
    ...
    "*** YOUR CODE HERE ***"
    num = n
    digit_link = Link(num%10)
    num //= 10
    while num > 0:
        digit_link = Link(num%10, digit_link)
        num //= 10
    return digit_link
