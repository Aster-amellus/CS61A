def berry_finder(t):
    ...
    if label(t) == 'berry':
        return True
    for elements in branches(t):
        if berry_finder(elements): 
            return True
    else:
        return False
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if is_leaf(t) and label(t) == 'loki':
        ...
        for i in branches(t):
            new_tree = replace_loki_at_leaf(i, lokis_replacement)
        return tree(label(t), new_tree)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    position = make_city('position', lat, lon)
    dis1 = distance(position, city_a)
    dis2 = distance(position, city_b)
    if dis1 > dis2:
...
def dejavu(t, n):
    ...
    if is_tree(t) == False:
        return False
    if n == 0 and is_leaf(t):
        return True
    else:
        return dejavu(branches(t), n-label(t))
