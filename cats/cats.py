...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    select_lst = [i for i in paragraphs if select(i)]
    if k>=len(select_lst):
        return ''
    else:
        return select_lst[k]
...
def about(subject):
    ...
    # END PROBLEM 2
    def is_in_obj(text):
        subjects = [lower(i) for i in subject]
        sum = 0
        text_lst = split(remove_punctuation(text))
        for i in text_lst:
            sum += lower(i) in subjects
        return not (sum == 0)
    return is_in_obj
...
def accuracy(typed, source):
    ...
    if typed == '' and source == '':
        ...
        minlen = min(len(typed_words), len(source_words))
        same_num = 0
        for i in range(minlen):
            same_num += typed_words[i] == source_words[i]
        percntage = round((same_num / len(typed_words))*100, 2)
        return percntage
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    len_typed = len(typed)
    words = len_typed / 5
    speed = round(words * (60/elapsed), 2)
    return speed
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    minlen = min(len(typed), len(source))
    maxlen = max(len(typed), len(source))
    count = 0
    for i in range(minlen):
        if typed[i] == source[i]:
            count += 1
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            tmplst.append(times[j][i])
        each_words_time.append(tmplst)
    fastest_lst = [[] for i in player_indices]
    for i in range(len(each_words_time)):
        base = each_words_time[i][0]
        index = 0
        for j in range(1,len(get_all_times(match))):
            if each_words_time[i][j] < base:
                base = each_words_time[i][j]
                index = j
        fastest_lst[index].append(words[i])
    return fastest_lst
