...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        if isinstance(expressions.rest.first, int):
        ...
        lambda_proc = do_lambda_form(Pair(expressions.first.rest, expressions.rest), env)
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.first is False:
            return False
        elif expressions.first is True:
            pass
        elif isinstance(expressions.first, (int,float)):
            flag = expressions.first 
        elif isinstance(expressions.first, Pair):
            flag = scheme_eval(expressions.first, env)
        elif isinstance(expressions.first, str):
            if env.lookup(expressions.first) is False:
                return False
            flag = env.lookup(expressions.first)
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.first is False:
            ...
            a = env.lookup(expressions.first)
            if a is False:
                pass
            elif a is True:
                flag = True
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        bind_exp = bindings.first 
        validate_form(bind_exp, 2, 2)
        names = Pair(bind_exp.first, names)
        vals = Pair(scheme_eval(bind_exp.rest.first, env), vals)
