...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        best_card = None
        if len(player.hand) > 0:
            temp = player.hand[0].power
            index = 0
            for i in range(len(player.hand)):
                if player.hand[i].power>temp:
                    temp = player.hand[i].power
                    index = i
            best_card = True
        if best_card == True:
            self.power += player.hand[i].power
            self.defense += player.hand[i].defense
            player.hand.draw(index)
        # You should add your implementation above this.
        if best_card:
